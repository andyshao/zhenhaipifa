...
namespace pifa.BLL {
	...
	public partial class Expressdesc {
		...
		public static int DeleteByExpress_id(uint? Express_id) {
			return dal.DeleteByExpress_id(Express_id);
		}
		...
		public static ExpressdescInfo GetItem(uint Express_id) {
			if (itemCacheTimeout <= 0) return Select.WhereExpress_id(Express_id).ToOne();
			string key = string.Concat("pifa_BLL_Expressdesc_", Express_id);
			string value = RedisHelper.Get(key);
			if (!string.IsNullOrEmpty(value))
				try { return ExpressdescInfo.Parse(value); } catch { }
			ExpressdescInfo item = Select.WhereExpress_id(Express_id).ToOne();
