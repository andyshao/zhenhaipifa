...
namespace pifa.DAL {
	...
	public partial class Member_fav_shop : IDAL {
		#region transact-sql define
		public string Table { get { return TSQL.Table; } }
		public string Field { get { return TSQL.Field; } }
		public string Sort { get { return TSQL.Sort; } }
		internal class TSQL {
			internal static readonly string Table = "`member_fav_shop`";
			...
			public static readonly string Delete = "DELETE FROM `member_fav_shop` WHERE ";
			public static readonly string Insert = "INSERT INTO `member_fav_shop`(`member_id`, `shop_id`, `create_time`) VALUES(?member_id, ?shop_id, ?create_time)";
		}
		#endregion
		...
		protected static MySqlParameter[] GetParameters(Member_fav_shopInfo item) {
		...
		public Member_fav_shopInfo GetItem(IDataReader dr) {
			int index = -1;
			return GetItem(dr, ref index) as Member_fav_shopInfo;
		}
		public object GetItem(IDataReader dr, ref int index) {
			return new Member_fav_shopInfo {
		...
		public SelectBuild<Member_fav_shopInfo> Select {
			get { return SelectBuild<Member_fav_shopInfo>.From(this, SqlHelper.Instance); }
		}
		#endregion
		...
		public int Update(Member_fav_shopInfo item) {
			return new SqlUpdateBuild(null, item.Member_id, item.Shop_id)
				.SetCreate_time(item.Create_time).ExecuteNonQuery();
		}
		#region class SqlUpdateBuild
		public partial class SqlUpdateBuild {
			protected Member_fav_shopInfo _item;
			protected string _fields;
			protected string _where;
			protected List<MySqlParameter> _parameters = new List<MySqlParameter>();
			public SqlUpdateBuild(Member_fav_shopInfo item, uint? Member_id, uint? Shop_id) {
				_item = item;
			...
			public override string ToString() {
				...
				if (string.IsNullOrEmpty(_where)) throw new Exception("防止 pifa.DAL.Member_fav_shop.SqlUpdateBuild 误修改，请必须设置 where 条件。");
			...
			public SqlUpdateBuild Set(string field, string value, params MySqlParameter[] parms) {
				if (value.IndexOf('\'') != -1) throw new Exception("pifa.DAL.Member_fav_shop.SqlUpdateBuild 可能存在注入漏洞，不允许传递 ' 给参数 value，若使用正常字符串，请使用参数化传递。");
		...
		public Member_fav_shopInfo Insert(Member_fav_shopInfo item) {
			SqlHelper.ExecuteNonQuery(TSQL.Insert, GetParameters(item));
			return item;
		}

		public Member_fav_shopInfo GetItem(uint? Member_id, uint? Shop_id) {
			return this.Select.Where("a.`member_id` = {0} AND a.`shop_id` = {1}", Member_id, Shop_id).ToOne();
		}
	}
}